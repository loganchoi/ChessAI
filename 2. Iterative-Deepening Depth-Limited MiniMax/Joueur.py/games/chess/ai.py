...
class AI(BaseAI):
    ...
    def make_move(self) -> str:
        ...
        move = (maxChoice(self.board,self.actual,inputs[1]))
        print(move)
        return move
        '''
...
#This just starts the recursive call for the functions and does IDDL
def maxChoice(board,moves,color):
    available = []
    v = -math.inf
    for depth in range(1,3):
        for m in moves:
            newBoard = deepcopy(board,m)
            val = minValue(newBoard,depth,color,color)
            if val == v:
                available.append(m)
            elif val > v:
                v = val
...
def maxValue(board,depth,color,ogColor):
    ...
    for move in valid_moves:
        ...
        val = minValue(newBoard,depth-1,switch_color[color],ogColor)
        if val >= v:
            v = val
...
def minValue(board,depth,color,ogColor):
    switch_color = {"b":"w","w":"b"}
    if terminal(board,ogColor):
        return h_value(board,ogColor)
    if depth == 0:
    ...
    for move in valid_moves:
        newBoard = deepcopy(board,move)
        val = maxValue(newBoard,depth-1,switch_color[color],ogColor)
        if val <= v:
            v = val
